...
class Normalized_Correlation_Layer(Layer):
    ...
    def get_output_shape_for(self, input_shape):
        ...
        cols = conv_output_length(inp_cols, self.kernel_size[1],
                                   self.border_mode, 1)
        out_r = conv_output_length(inp_rows, self.kernel_size[0],
                                   self.border_mode, self.subsample[0])
        out_c = conv_output_length(inp_cols, self.kernel_size[1],
        ...
        for i in range(output_row):
            for j in range(output_col):
                ...
                xc_2.append(K.reshape(input_2[:, slice_row, slice_col, :],
                                      (-1, 1, self.kernel_size[0]*self.kernel_size[1]*inp_shape[-1])))
